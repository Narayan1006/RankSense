...
with col_up:
    ...
    if uploaded:
        ...
        st.info("👆 Upload a `.jsonl` file to begin  \n*(Use the sample test dataset or full 100k candidates)*")
...
if rank_clicked:
    # Validate inputs
    if not uploaded:
        st.error("⚠️ Please upload a candidates `.jsonl` file first!")
        st.stop()
    ...
    # Show sample output when button hasn't been clicked
    st.markdown("""
